...
func connect_to_host(url: String, secure_connection_method: SecureConnectionMethod = SecureConnectionMethod.NONE, _connect_timeout := 30) -> int:
	...
	var regex = RegEx.new()
	# Improved regex: handles any characters in user, password, host, port, and dbname
	# Uses character classes to avoid greedy issues
	var pattern = "^(?:postgresql|postgres)://([^:]+):([^@]+)@([^:]+):(\\d*)/(.+)$"
	var compile_err = regex.compile(pattern)
	if compile_err != OK:
		push_error("[PostgreSQLClient:%d] RegEx compilation failed: %d" % [get_instance_id(), compile_err])
	...
	if result:
		...
		if error == OK:
			next_etape = true
			status = Status.STATUS_CONNECTING   # <-- FIX: Set connecting state
		else:
				...
				if not response[1].is_empty():
					match char(response[1][0]):
						'S':
							...
							stream_peer_tls.connect_to_stream(peer, "", tls_options)
							status_ssl = 2
						'N':
							status = Status.STATUS_ERROR
		...
		if status == Status.STATUS_CONNECTED and busy:
			...
			if stream_peer_tls.get_status() == stream_peer_tls.STATUS_CONNECTED:
				if stream_peer_tls.get_available_bytes():
					response = stream_peer_tls.get_data(stream_peer_tls.get_available_bytes())
			else:
	...
	while response_buffer.size() > 4:
		...
		match char(response_buffer[0]):
			...
			'N':
				...
				var last_datas_command_sql = datas_command_sql.back() if not datas_command_sql.is_empty() else postgresql_query_result_instance
			...
			'T':
				...
				for _index in postgresql_query_result_instance.number_of_fields_in_a_row:
					...
					# Advance past field name + null terminator
					cursor += len(field_name) + 1
					...
					var column_index = response_buffer.slice(cursor + 4, cursor + 6)
					...
					var type_object_id = response_buffer.slice(cursor + 6, cursor + 10)
					...
					var data_type_size = response_buffer.slice(cursor + 10, cursor + 12)
					...
					var type_modifier = response_buffer.slice(cursor + 12, cursor + 16)
					...
					var format_code = response_buffer.slice(cursor + 16, cursor + 18)
					format_code.reverse()
					...
					# Advance cursor past all fixed-width fields (18 bytes total)
					cursor += 18
